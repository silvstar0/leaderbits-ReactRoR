...
window.UnobtrusiveFlash.flashOptions = { type: 'notice', timeout: 6000 };
...
(function() {
  ...
  UnobtrusiveFlash.showFlashMessage = function(message, options) {
    ...
    var $flash = $(
      '<div class="unobtrusive-flash-message-wrapper unobtrusive-flash-' +
        options.type +
        '"><div class="unobtrusive-flash-message">' +
        message +
        '</div></div>'
    );
    ...
    if ($flashContainer.length == 0) {
      $flashContainer = $('<div/>')
        .attr('id', 'unobtrusive-flash-messages')
        .prependTo('body');
    }
